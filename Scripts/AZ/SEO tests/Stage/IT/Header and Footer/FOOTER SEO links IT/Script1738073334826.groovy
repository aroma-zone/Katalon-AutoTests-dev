...
List<String> footerlinks = footer.select("a[href]").eachAttr("href")

KeywordUtil.logInfo("Nombre de liens extraits : ${footerlinks.size()}")
if (footerlinks.isEmpty()) {
    KeywordUtil.markFailedAndStop("Aucun lien n'a été extrait dans le footer. Vérifiez vos sélecteurs CSS.")
}
...
File file2 = new File("C:\\LINKS SEO FR IT\\verified_links_footer_it.txt")
file2.text = "Vérification des liens du footer IT - ${new Date()}\n"

footerlinks.each { String link ->
    try {
...
if (testFailed) {
    KeywordUtil.markFailedAndStop("Le test a échoué car un ou plusieurs liens dans le footer ont retourné un code non-200.")
} else {
    KeywordUtil.logInfo("Tous les liens du footer ont été vérifiés avec succès.")
}
