...
WebUI.setText(findTestObject('AZ/Components/Search bar/input_search titem'), 'stetegegdssd')
...
String script = """
    return window.dataLayer.find(event => 
        event.event === 'search' && 
        event.event_name === 'empty_search' && 
        event.event_label === 'empty_search' 
    );
"""
...
assert event != null : 'L\'événement de tracking "search" avec "empty_search" n\'a pas été trouvé dans la couche de données.'

assert event.event_name == 'empty_search' : 'L\'événement trouvé n\'est pas "search".'

println('L\'événement de tracking \'search\' avec \'empty_search\' a été trouvé avec succès dans la couche de données.')
